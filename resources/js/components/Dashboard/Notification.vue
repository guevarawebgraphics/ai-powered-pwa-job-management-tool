...
export default {
    ...
    computed: {
        notifyType() {
            return this.activeTab === "Gigs" ? 1 :
                this.activeTab === "Guild" ? 2 : 3;
        }
    },
    methods: {
        async userData() {
        ...
        async fetchNotifications() {
            try {
                ...
                const response = await axios.get(`/api/notify/${userId}/${this.notifyType}`);

                if (response.data && response.data.data.length > 0) {
                    this.notifications = this.formatNotifications(response.data.data);
                } else {
        ...
        async markAsRead(notification) {
            ...
            try {
                await axios.post(`/api/notify/update/${notification.id}`, { is_seen: 1 });
