                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" v-model="notificationsEnabled"
                                @change="updateSettings('is_notify', notificationsEnabled)" class="sr-only">
                            <div class="relative w-10 h-5 bg-gray-300 rounded-full transition-all duration-300"
                    ...
                    <div class="flex justify-between items-center mt-3">
                        ...
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" v-model="locationEnabled"
                                @change="updateSettings('is_location', locationEnabled)" class="sr-only">
                            <div class="relative w-10 h-5 bg-gray-300 rounded-full transition-all duration-300"
    ...
    <div v-if="isChatOpen" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center">
        <div class="bg-white w-96 rounded-lg shadow-lg">
            ...
            <div class="flex justify-between items-center bg-blue-500 text-white p-4 rounded-t-lg">
                ...
                <button @click="closeChat" class="text-white text-xl">&times;</button>
            </div>

            <!-- Chat Body -->
            <div class="p-4 h-64 overflow-y-auto" ref="chatBox">
                <div v-for="(message, index) in messages" :key="index" class="flex items-start mb-3"
                    :class="{ 'justify-end': message.type === 'sender', 'justify-start': message.type === 'receiver' }">

                    <!-- Profile Picture -->
                    <img :src="message.avatar" alt="Profile" class="w-8 h-8 rounded-full"
                        :class="{ 'order-2 ml-2': message.type === 'sender', 'mr-2': message.type === 'receiver' }">

                    <!-- Message Content -->
                    <div>
                        <p class="text-xs font-bold"
                            :class="{ 'text-right': message.type === 'sender', 'text-left': message.type === 'receiver' }">
                            {{ message.name }}
                        </p>
                        <div :class="message.type === 'sender' ? 'bg-[#F3F4F6FF] text-[#171A1FFF]' : 'bg-[#E5EDF8FF] text[#171A1FFF]'"
                ...
                .then(response => {
                    ...
                    this.messages = response.data.data.map(msg => {
                        ...
                        return {
                            ...
                            avatar: isSentByUser
                                ? (this.accountProfile.profile_photo || "https://randomuser.me/api/portraits/men/20.jpg")
                                : (msg.sender.profile_photo || `https://theguild.appliancerepairamerican.com/public/images/logo.png`)
                        };
                    });
